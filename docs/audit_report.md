...
* **Reviewer Note:** While using the `@shopify/polaris` React library is not strictly mandatory, the app interface *must* feel native to the Shopify admin. If your custom Tailwind UI deviates significantly from Shopify's design language, it could lead to rejection under the "User Experience" guidelines.

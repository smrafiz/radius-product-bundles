...
import { getBundlesServer } from "@/actions/bundles.action";
...
interface BundlesState {
    ...
    // Server fetch
    fetchBundles: (token: string) => void;
    debouncedFetchBundles: (token: string) => void;
}
...
export const useBundlesStore = create<BundlesState>()((set, get) => ({
    ...
    // Server fetch action
    fetchBundles: async (token: string) => {
        const {
            filters,
            pagination,
            setLoading,
        ...
        try {
            ...
            const res = await getBundlesServer(token, {
                search: filters.search,
                statusFilter: filters.statusFilter,
                typeFilter: filters.typeFilter,
                sort: filters.sortSelected,
            ...
            if (res.status === "success") {
                ...
                const msg = res.message || "Failed to fetch bundles";
                setError(msg);
                showToast(msg);
            }
        } catch (err) {
            console.error("fetchBundles error:", err);
            const msg = "Failed to fetch bundles";
            setError(msg);
            showToast(msg);
        } finally {
            setLoading(false);
    ...
    // Debounced version
    debouncedFetchBundles: debounce(fetchBundles, 400),
}));

...
export const useBundlesStore = create<BundlesState>()((set, get) => ({
    ...
    getFilteredBundles: () => {
        ...
        const itemStrings = ['ALL', 'ACTIVE', 'DRAFT', 'PAUSED', 'SCHEDULED', 'ARCHIVED'];
        ...
        try {
            ...
            if (filters.search && filters.search.trim()) {
                ...
                filtered = filtered.filter(bundle => 
                    bundle?.name?.toLowerCase()?.includes(searchTerm) || false
                );
            }
            ...
            // Tab filter
            if (filters.selectedTab > 0 && filters.selectedTab < itemStrings.length) {
                const tabStatus = itemStrings[filters.selectedTab];
                if (tabStatus && tabStatus !== 'ALL') {
                    filtered = filtered.filter(bundle => 
                        bundle?.status === tabStatus
                    );
                }
            }
            ...
            if (filters.sortSelected && filters.sortSelected.length > 0) {
                const [sortKey, sortDirection] = filters.sortSelected[0]?.split(' ') || ['created_at', 'desc'];
                filtered.sort((a, b) => {
                    let aVal, bVal;
                    switch (sortKey) {
                        case 'name':

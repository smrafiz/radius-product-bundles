...
describe('BundleRepository', () => {
    ...
    describe('findByShop', () => {
        it('should find bundles by shop', async () => {
            ...
            const result = await bundleRepository.findByShop('test-shop.myshopify.com');

            expect(result).toEqual(mockBundles);
            expect(prismaMock.bundle.findMany).toHaveBeenCalledWith(
                expect.objectContaining({
                    where: expect.objectContaining({ shop: 'test-shop.myshopify.com' }),
                })
            );
        });
    });
    ...
    describe('countByShop', () => {
        it('should count bundles by shop', async () => {
            ...
            const count = await bundleRepository.countByShop('test-shop.myshopify.com');

...
export function useProductMediaPicker() {
    ...
    const { addSelectedProductMediaUrls } = useBundleStore();

    /**
     * Add selected images (existing Shopify URLs)
     * No file conversion needed - we'll attach these directly
     */
    const addImages = (imageUrls: string[]) => {
        ...
        try {
            addSelectedProductMediaUrls(imageUrls);
        } catch (error) {
            console.error("Failed to add images:", error);
        } finally {
            setIsLoading(false);

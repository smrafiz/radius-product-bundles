...
export default function EditBundlePage({ params }: EditBundlePageProps) {
    ...
    // Fetch bundle data
    const { data: bundleResult, isLoading, error } = useQuery({
            ...
            <Card>
                <div style={{ padding: '20px' }}>
                    <Banner tone="critical" title="Error loading bundle">
                        <Text variant="bodyMd">
                            {bundleResult?.message || 'Failed to load bundle. Please try again.'}
                        </Text>
                    </Banner>
                </div>
            </Card>
    ...
    const initialData = bundleResult?.data ? {
        ...
        startDate: bundleResult.data.startDate ? new Date(bundleResult.data.startDate) : undefined,
        endDate: bundleResult.data.endDate ? new Date(bundleResult.data.endDate) : undefined,

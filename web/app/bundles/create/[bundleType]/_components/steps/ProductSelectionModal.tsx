...
import { SearchIcon, FilterIcon, ChevronDownIcon, ChevronUpIcon } from '@shopify/polaris-icons';
import { useGraphQL } from '@/hooks/useGraphQL';
import { request } from "graphql-request";
import { LATEST_API_VERSION } from "@shopify/shopify-api";

// Import your existing GraphQL documents
import { GetProductsDocument, GetCollectionsForFiltersDocument } from '@/lib/gql/graphql';
...
export default function ProductSelectionModal({
    ...
    const [filterPopoverActive, setFilterPopoverActive] = useState<boolean>(false);
    ...
    const buildSearchQuery = useCallback(() => {
        ...
        // Add filters
        if (filters.status) {
            query += query ? ` AND status:${filters.status}` : `status:${filters.status}`;
        } else {
            query += query ? ` AND status:ACTIVE` : `status:ACTIVE`;
        }

        if (filters.productType) {
            query += query ? ` AND product_type:"${filters.productType}"` : `product_type:"${filters.productType}"`;
        }
        if (filters.vendor) {
            query += query ? ` AND vendor:"${filters.vendor}"` : `vendor:"${filters.vendor}"`;
        }
        if (filters.collection) {
            query += query ? ` AND collection_id:${filters.collection}` : `collection_id:${filters.collection}`;
        }
    ...
    const allProductsQuery = useGraphQL<GetProductsQuery, GetProductsQueryVariables>(
        GetProductsDocument,
    ...
    const productsQuery = useGraphQL<GetProductsQuery, GetProductsQueryVariables>(
        GetProductsDocument,
        productsVariables
    );
    ...
    const loadMoreProducts = useCallback(async () => {
        ...
        try {
            ...
            if (newProducts.length > 0) {
                const transformedProducts = newProducts.map((edge: any) => {
                    ...
                    return {
                        ...
                        variants: node.variants.edges.map((variantEdge: any) => ({
                            ...
                            sku: variantEdge.node.sku || undefined,
                            price: variantEdge.node.price,
                            compareAtPrice: variantEdge.node.compareAtPrice || undefined,
                            availableForSale: variantEdge.node.availableForSale,
                            inventoryQuantity: variantEdge.node.inventoryQuantity,
                            image: variantEdge.node.image ? {
                                url: variantEdge.node.image.url,
                                altText: variantEdge.node.image.altText || undefined,
                            } : undefined,
                ...
                setAllLoadedProducts(prev => [...prev, ...transformedProducts]);
    ...
    const collectionsQuery = useGraphQL<GetCollectionsForFiltersQuery, GetCollectionsForFiltersQueryVariables>(
        ...
        if (productsQuery.data?.products?.edges) {
            const transformedProducts = productsQuery.data.products.edges.map(edge => {
                ...
                return {
                    ...
                    collections: node.collections.edges.map(collectionEdge => ({
                        id: collectionEdge.node.id,
                        title: collectionEdge.node.title,
                    })),
                };
            ...
            // Reset loaded products when search/filter changes
    ...
    const handleProductToggle = useCallback((product: Product) => {
        const productSelected = selectedItems.some(item =>
    ...
    const handleVariantToggle = useCallback((product: Product, variant: ProductVariant) => {
        const variantSelected = selectedItems.some(item =>
            item.type === 'variant' && item.variantId === variant.id
        );

        if (variantSelected) {
            setSelectedItems(prev => prev.filter(item => item.variantId !== variant.id));
        } else {
    ...
    const isProductSelected = useCallback((product: Product) => {
        return selectedItems.some(item =>
            item.type === 'product' && item.productId === product.id
        ) || product.variants.every(variant =>
            selectedItems.some(item => item.variantId === variant.id)
        );
    }, [selectedItems]);

    const isVariantSelected = useCallback((variant: ProductVariant) => {
                                ...
                                <InlineStack gap="200" align="space-between">
                                    ...
                                    <Select
                                        options={[
                                            ...
                                            { label: "Search by SKU", value: "sku" },
                                            { label: "Search by Barcode", value: "barcode" },
                                        ]}
                                        value={searchBy}
                                        ...
                                        <div style={{ padding: "16px", minWidth: "300px" }}>
                                            <BlockStack gap="300">
                                                ...
                                                {collectionsQuery.data?.collections?.edges && (
                                                    <BlockStack gap="200">
                                                        <Text variant="bodyMd" fontWeight="medium">Collections</Text>
                                                        <BlockStack gap="100">
                                                            {collectionsQuery.data.collections.edges.slice(0, 5).map(edge => (
                                                                <RadioButton
                                                                    key={edge.node.id}
                                                                    ...
                                                                    onChange={() => handleFilterChange("collection",
                                                                        filters.collection === edge.node.id ? "" : edge.node.id
                                                                    )}
                                                                />
                                                            ))}
                                                        </BlockStack>
                                    ...
                                    <BlockStack gap="0">
                                        {products.map((product, index) => (
                                            <React.Fragment key={product.id}>
                                                ...
                                                {/* Variants */}
                                                {product.variants.length > 1 && expandedProducts.has(product.id) && (
                                                    <Box paddingInlineStart="600" paddingInlineEnd="300" paddingBlockEnd="200">
                                                        <BlockStack gap="100">
                                                            {product.variants.map((variant) => (
                                                                <Box key={variant.id} padding="200" background="bg-surface-secondary">
                                                                    <InlineStack align="space-between" blockAlign="center">
                                                                        <InlineStack gap="200" blockAlign="center">
                                                                            <Checkbox
                                                                                checked={isVariantSelected(variant)}
                                                                                onChange={() => handleVariantToggle(product, variant)}
                                                                            />
                                                                        ...
                                                                        <BlockStack gap="050" inlineAlign="end">
                                                                            ...
                                                                            {variant.compareAtPrice && parseFloat(variant.compareAtPrice) > parseFloat(variant.price) && (
                                                                                <Text variant="caption" tone="subdued" textDecorationLine="line-through">
